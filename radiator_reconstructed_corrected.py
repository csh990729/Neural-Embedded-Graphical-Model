...
# =============================================================================
def build_fine_mesh(n_coarse, n_fine):
    dx_fine = L / n_fine
...
def verify_mesh(nodes, elements, name="mesh"):
    import numpy as np
    node_count = np.zeros(len(nodes), dtype=int)
    for conn in elements:
        node_count[np.asarray(conn)] += 1
    orphan = np.where(node_count == 0)[0]
    print(f"{name}: nodes={len(nodes)}, elements={len(elements)}, orphan_nodes={len(orphan)}")
    return orphan
...
if __name__ == "__main__":
    ...
    verify_mesh(coarse_nodes, coarse_elems, 'coarse')
    ...
    verify_mesh(fine_nodes, fine_elems, 'fine')
